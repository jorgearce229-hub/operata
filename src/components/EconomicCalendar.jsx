...
export default function EconomicCalendar() {
  ...
  const parseImp = i => { const n = parseInt(i); if (n >= 2) return 'high'; if (n === 1) return 'medium'; return 'low' }
  ...
  const fetchCalendar = async () => {
    ...
    try {
      ...
      if (!res.ok) { setError(`Error ${res.status} — función serverless no disponible`); setLoading(false); return }
      const data = await res.json()
      if (!data.status || !data.response) { setError(`Error de API: ${JSON.stringify(data).slice(0,150)}`); setLoading(false); return }
      const parsed = data.response.map(e => ({
        id: e.id, date: (e.date||'').slice(0,10), time: (e.date||'').slice(11,16),
        event: e.title || e.indicator || '', currency: e.currency || e.country || '',
        impact: parseImp(e.importance), previous: String(e.previous||''),
        forecast: String(e.forecast||''), actual: String(e.actual||''),
      })).filter(e => e.date && e.event).sort((a,b) => (a.date+a.time)<(b.date+b.time)?-1:1)
      setEvents(parsed)
  ...
  const todayMedCurr = new Set(events.filter(e=>e.date===todayStr&&e.impact==='medium').map(e=>e.currency))
  const CURRENCY_NAMES = { USD:'Dólar (USD)', EUR:'Euro (EUR)', GBP:'Libra (GBP)', JPY:'Yen (JPY)', CHF:'Franco Suizo (CHF)', CAD:'Dólar Canadiense (CAD)', AUD:'Dólar Australiano (AUD)', NZD:'Dólar Neozelandés (NZD)', CNY:'Yuan (CNY)', MXN:'Peso Mexicano (MXN)' }
      ...
      {!loading && events.length > 0 && (
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:'16px'}}>
          <div style={{...card,border:`1px solid ${C.green}30`,marginBottom:0}}>
            <div style={{fontSize:'12px',fontWeight:600,color:C.muted,textTransform:'uppercase',letterSpacing:'0.8px',marginBottom:'8px'}}>✅ Monedas Sin Eventos Hoy</div>
            {todayHighCurr.size===0
              ?<div style={{color:C.green,fontSize:'13px',fontWeight:600}}>Sin eventos de alto impacto hoy — todas las monedas libres</div>
              :<><div style={{fontSize:'11px',color:C.dim,marginBottom:'6px'}}>Sin eventos de alto impacto:</div>
                <div style={{display:'flex',flexWrap:'wrap',gap:'5px'}}>
                  {Object.keys(CURRENCY_NAMES).filter(c=>!todayHighCurr.has(c)).map(c=><span key={c} style={B('g')}>{c}</span>)}
                </div></>}
          </div>
          <div style={{...card,border:`1px solid ${C.red}30`,marginBottom:0}}>
            <div style={{fontSize:'12px',fontWeight:600,color:C.muted,textTransform:'uppercase',letterSpacing:'0.8px',marginBottom:'8px'}}>🚫 Monedas con Eventos Hoy</div>
            {todayHighCurr.size===0
              ?<div style={{color:C.muted,fontSize:'13px'}}>Ninguna moneda afectada hoy</div>
              :<><div style={{fontSize:'11px',color:C.dim,marginBottom:'6px'}}>Evita instrumentos que contengan estas monedas:</div>
                <div style={{display:'flex',flexWrap:'wrap',gap:'6px'}}>
                  {[...todayHighCurr].map(c=><div key={c} style={{display:'flex',alignItems:'center',gap:'4px',padding:'4px 10px',borderRadius:'6px',background:C.redBg,border:`1px solid ${C.red}30`}}>
                    <span style={{width:'8px',height:'8px',borderRadius:'50%',background:C.red,display:'inline-block',flexShrink:0}}/>
                    <span style={{fontSize:'12px',fontWeight:700,color:C.red}}>{c}</span>
                    <span style={{fontSize:'11px',color:C.muted}}>alto impacto</span>
                  </div>)}
                  {[...todayMedCurr].filter(c=>!todayHighCurr.has(c)).map(c=><div key={c} style={{display:'flex',alignItems:'center',gap:'4px',padding:'4px 10px',borderRadius:'6px',background:C.yellowBg,border:`1px solid ${C.yellow}30`}}>
                    <span style={{width:'8px',height:'8px',borderRadius:'50%',background:C.yellow,display:'inline-block',flexShrink:0}}/>
                    <span style={{fontSize:'12px',fontWeight:700,color:C.yellow}}>{c}</span>
                    <span style={{fontSize:'11px',color:C.muted}}>medio impacto</span>
                  </div>)}
                </div>
                <div style={{marginTop:'8px',fontSize:'11px',color:C.dim}}>Aplica a cualquier par, acción, cripto o futuro que contenga estas divisas</div>
              </>}
          </div>
        </div>
      )}
              ...
              return <div key={date} style={{display:'flex',borderBottom:`1px solid ${C.border}20`,paddingBottom:'4px',marginBottom:'4px'}}>
                ...
                <div style={{flex:1}}>
                  {dayEvs.map((e,i)=>{
                    ...
                    const aff=[]  // pairs shown are user-specific, not hardcoded
                    return <div key={i}>
                      ...
                      {isExp&&<div style={{padding:'10px 14px',marginBottom:'4px',background:C.card,borderRadius:'6px',border:`1px solid ${C.border}`}}>
                        ...
                        {e.currency&&<div style={{fontSize:'11px',color:C.dim,marginTop:'4px'}}>Revisa tus instrumentos con <b style={{color:e.impact==='high'?C.red:C.yellow}}>{e.currency}</b></div>}
                        {e.impact==='high'&&<div style={{marginTop:'8px',fontSize:'11px',color:C.red,fontWeight:600}}>⚠ Alto impacto — evita nuevas posiciones en estos pares hoy</div>}
                      </div>}
                    </div>
                  })}
                </div>
