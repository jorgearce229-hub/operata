                  ...
                  {dayEvs.map((e,i)=>{
                    ...
                    return <div key={i}>
                      ...
                      {isExp&&<div style={{padding:'10px 14px',marginBottom:'4px',background:C.card,borderRadius:'6px',border:`1px solid ${C.border}`}}>
                        ...
                        {aff.length>0&&<><div style={{fontSize:'11px',color:C.dim,marginBottom:'5px'}}>Pares afectados: </div>
                          <div style={{display:'flex',flexWrap:'wrap',gap:'5px'}}>{aff.map(p=><span key={p} style={B(e.impact==='high'?'r':'b')}>{p}</span>)}</div></>}
