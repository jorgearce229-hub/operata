...
function ClockLive() {
  ...
  return <span>{time.toLocaleTimeString('es-MX',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}</span>
}
...
export default function Journal({ trades, userId, onUpdate, isPro, tradeCount, profile }) {
  ...
  const saveTrade = async () => {
    ...
    setForm(empty); setShowForm(false); onUpdate(); setSaving(false)
  }
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px' }}>
        <div>
          ...
          <div style={{ fontSize:'12px', color:C.dim, marginTop:'3px' }}>
            {new Date().toLocaleDateString('es-MX',{weekday:'long',day:'numeric',month:'long',year:'numeric'})} · <ClockLive/>
          </div>
        </div>
