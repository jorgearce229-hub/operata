...
export default function Journal({ trades, userId, onUpdate, isPro, tradeCount, profile, accountId }) {
  ...
  const canAdd = (isPro || tradeCount < FREE_TRADE_LIMIT) && accountId !== 'all'
  ...
  const saveTrade = async () => {
    ...
    if (screenshotFileRef.current) {
      screenshotUrl = await uploadScreenshot(screenshotFileRef.current)
    }
    const { error } = await supabase.from('trades').insert({
      user_id: userId, account_id: accountId || null, ...form,
...
{isPro && accountId !== 'all' && <button style={{...btn("ghost"),fontSize:"13px"}} onClick={()=>{setShowImport(!showImport);setShowForm(false)}}>{showImport ? "✕ Cancelar" : "📥 Importar CSV"}</button>}
      ...
      {!canAdd && accountId === 'all' && (
        <div style={{ ...card, textAlign:'center', padding:'40px', border:`1px solid ${C.border}` }}>
          <div style={{ fontSize:'32px', marginBottom:'12px' }}>🌐</div>
          <div style={{ fontSize:'15px', fontWeight:600, marginBottom:'6px' }}>Estás viendo todas tus cuentas combinadas</div>
          <div style={{ color:C.muted, fontSize:'13px' }}>Selecciona una cuenta específica en "Cuentas" para registrar una nueva operación.</div>
        </div>
      )}
      ...
      {isPro && showImport && <ImportTrades userId={userId} accountId={accountId} onImported={() => { setShowImport(false); onUpdate() }} />}
      {showForm && canAdd && (
      ...
      {Object.keys(grouped).length === 0 && !showForm ? (
        <div style={{ ...card, textAlign:'center', padding:'60px' }}>
          ...
          <div style={{ fontSize:'17px', fontWeight:700, marginBottom:'6px' }}>Todavía no tienes operaciones registradas</div>
          <div style={{ color:C.muted, fontSize:'13px', marginBottom:'20px', maxWidth:'360px', marginLeft:'auto', marginRight:'auto' }}>
            Registra tu primera operación para empezar a ver tu win rate, curva de equity y estadísticas en Analytics.
          </div>
          <div style={{ display:'flex', gap:'10px', justifyContent:'center', flexWrap:'wrap' }}>
            {canAdd && <button style={btn()} onClick={()=>setShowForm(true)}>+ Registrar mi primera operación</button>}
            {isPro && accountId !== 'all' && <button style={btn('ghost')} onClick={()=>setShowImport(true)}>📥 Importar desde CSV</button>}
          </div>
        </div>
