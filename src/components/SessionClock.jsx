...
export default function SessionClock({ userId }) {
  ...
  const localH = now.getHours(), localM = now.getMinutes(), localS = now.getSeconds()
        ...
        <div style={card}>
          ...
          {sessions.map((s,i) => (
            <div key={i} style={{display:'flex',alignItems:'center',gap:'12px',padding:'10px 0',borderBottom:`1px solid ${C.border}`}}>
              ...
              <input style={{...require('../lib/theme').input,width:'120px'}} value={s.name} onChange={e=>{const ns=[...sessions];ns[i]={...ns[i],name:e.target.value};setSessions(ns)}} />
              <span style={{color:C.dim,fontSize:'12px'}}>UTC</span>
              <input style={{...require('../lib/theme').input,width:'60px',textAlign:'center'}} type="number" value={s.start} onChange={e=>{const ns=[...sessions];ns[i]={...ns[i],start:parseInt(e.target.value)||0};setSessions(ns)}} />
              <span style={{color:C.dim}}>→</span>
              <input style={{...require('../lib/theme').input,width:'60px',textAlign:'center'}} type="number" value={s.end} onChange={e=>{const ns=[...sessions];ns[i]={...ns[i],end:parseInt(e.target.value)||0};setSessions(ns)}} />
