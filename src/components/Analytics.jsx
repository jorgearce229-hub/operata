import { C, card, badge } from '../lib/theme'
...
export default function Analytics({ trades, isPro }) {
  ...
  let cum=0
  const eqData = sorted.map(t=>{cum+=(t.pnl||0);return{name:(t.entry_date||'').slice(5),pnl:Math.round((t.pnl||0)*100)/100,equity:Math.round(cum*100)/100}})
  ...
  if (sorted.length === 0) return (
    <div style={{textAlign:'center',padding:'80px 20px'}}>
      ...
      <div style={{fontSize:'15px',fontWeight:600,marginBottom:'6px'}}>Sin datos para analizar</div>
      <div style={{color:C.muted,fontSize:'13px'}}>Registra operaciones en el Diario para ver tu analytics</div>
    </div>
  )
  ...
  return (
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))',gap:'12px',marginBottom:'16px'}}>
        <Stat label="P&L Total" value={`${totalPnL>=0?'+':''}$${totalPnL.toFixed(2)}`} color={totalPnL>=0?C.green:C.red} sub={`${sorted.length} operaciones`} />
        <Stat label="Win Rate" value={`${wr}%`} color={wr>=50?C.green:C.yellow} sub={`${wins}W / ${losses}L`} />
        <Stat label="Expectancy" value={`${avgR>=0?'+':''}${avgR.toFixed(2)}R`} color={avgR>=0?C.green:C.red} sub="por operación" />
        <Stat label="Max Drawdown" value={`${maxDD.toFixed(1)}%`} color={C.red} />
        <Stat label="Mejor Trade" value={`+$${Math.max(0,...sorted.map(t=>t.pnl||0)).toFixed(2)}`} color={C.green} />
        <Stat label="Peor Trade" value={`-$${Math.abs(Math.min(0,...sorted.map(t=>t.pnl||0))).toFixed(2)}`} color={C.red} />
      </div>
      ...
      <div style={card}>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={eqData} margin={{top:5,right:20,bottom:5,left:10}}>
            ...
            <Tooltip content={<Tooltip2/>}/><ReferenceLine y={0} stroke={C.dim} strokeDasharray="4 4"/>
            <Area type="monotone" dataKey="equity" name="Capital" stroke={C.accent} fill="url(#eG)" strokeWidth={2} dot={false}/>
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      {isPro ? (
        ...
        <div style={{...card, textAlign:'center', padding:'40px', border:`1px solid ${C.yellow}30`}}>
          ...
          <div style={{fontWeight:600,marginBottom:'4px'}}>Analytics avanzados</div>
          <div style={{color:C.muted,fontSize:'13px',marginBottom:'14px'}}>P&L mensual, análisis por instrumento y más — disponible en Pro</div>
