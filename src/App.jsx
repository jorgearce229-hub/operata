...
export default function App() {
  ...
  if (typeof window !== 'undefined' && window.location.search.includes('upgrade=success')) {
  ...
  if (showUpgrade) {
    return <Upgrade session={session} onClose={() => setShowUpgrade(false)} />
  }
  ...
  if (showSettings) {
    return <Settings 
      ...
      theme={theme}
      onThemeChange={(mode) => { setThemeState(mode); localStorage.setItem('operata-theme', mode) }}
      isPro={profile?.plan === 'pro'}
      userEmail={session.user.email}
      onClose={() => setShowSettings(false)}
    />
  }
  return <Dashboard session={session} profile={profile} onProfileUpdate={handleProfileUpdate} theme={theme} onThemeChange={(mode) => setThemeState(mode)} onShowSettings={() => setShowSettings(true)} onShowUpgrade={() => setShowUpgrade(true)} />
}
