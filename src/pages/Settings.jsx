...
function CancelSubscription({ subscriptionId, onCancelled, profile }) {
    ...
    <div style={{ padding: '14px', background: C.bg, border: `1px solid ${C.border}`, borderRadius: '10px' }}>
      {!confirm ? (
        ...
        <div>
          ...
          <div style={{ fontSize: '12px', color: C.muted, marginBottom: '12px' }}>Tu plan Pro seguirá activo hasta el final del período de facturación actual. No se realizarán más cargos.</div>
          {error && <div style={{ fontSize: '12px', color: C.red, marginBottom: '8px' }}>{error}</div>}
          <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={handleCancel} disabled={loading} style={{ padding: '7px 16px', borderRadius: '8px', border: 'none', background: C.red, color: '#fff', fontWeight: 600, fontSize: '12px', cursor: 'pointer' }}>
              {loading ? 'Cancelando...' : 'Sí, cancelar'}
            </button>
            <button onClick={() => setConfirm(false)} style={{ padding: '7px 16px', borderRadius: '8px', border: `1px solid ${C.border}`, background: 'transparent', color: C.muted, fontWeight: 600, fontSize: '12px', cursor: 'pointer' }}>
              No, mantener Pro
            </button>
          </div>
        </div>
    ...
    <div style={{ maxWidth: '700px', margin: '0 auto', padding: '24px' }} ref={containerRef}>
      ...
      <Section title="Cuenta">
        ...
        {profile?.plan === 'pro' && profile?.stripe_subscription_id && (
          <CancelSubscription subscriptionId={profile.stripe_subscription_id} onCancelled={onProfileUpdate} profile={profile} />
        )}
      </Section>
    </div>
    </div>
