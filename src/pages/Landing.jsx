...
const PLANS = [
  ...
  { name: 'Pro', price: '$9.99', period: '/mes', features: ['Operaciones ilimitadas', 'Analytics completos', 'Calendar heatmap', 'Backtesting avanzado', 'Importar / Exportar CSV', 'P&L mensual detallado'], cta: 'Empezar Pro', highlight: true },
]
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0b1120', color:'#e2e8f0', fontFamily:"'Inter', system-ui, sans-serif" }}>
      ...
      <section style={{ padding:'70px 20px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:'20px', maxWidth:'640px', margin:'0 auto' }}>
          {PLANS.map((p,i)=>(
            <div key={i} style={{ padding:'28px', background:p.highlight?'#06d6a018':'#111827', border:`2px solid ${p.highlight?'#06d6a0':'#1e293b'}`, borderRadius:'16px', position:'relative' }}>
              {p.highlight&&<div style={{ position:'absolute', top:'-13px', left:'50%', transform:'translateX(-50%)', background:'#06d6a0', color:'#0b1120', padding:'4px 16px', borderRadius:'20px', fontSize:'11px', fontWeight:700 }}>MÁS POPULAR</div>}
