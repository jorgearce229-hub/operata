...
const FEATURES = [
  ...
  {
    icon: '🏦', title: 'Múltiples Cuentas',
    desc: 'Lleva varias cuentas por separado, o velas todas combinadas en un solo lugar.',
    preview: (
      <div style={{fontSize:'11px'}}>
        {[{n:'Cuenta Real · IBKR',v:'$12,340.00',c:'#10b981'},{n:'Cuenta Demo',v:'$9,875.50',c:'#3b82f6'}].map((a,i)=>(
          <div key={i} style={{marginBottom:'6px',padding:'6px 8px',background:'#0b1120',borderRadius:'6px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <span style={{color:'#94a3b8'}}>{a.n}</span>
            <span style={{color:a.c,fontWeight:700}}>{a.v}</span>
          </div>
        ))}
        <div style={{marginTop:'6px',padding:'8px',background:'#06d6a018',border:'1px solid #06d6a030',borderRadius:'6px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <span style={{color:'#06d6a0',fontSize:'10px',fontWeight:600}}>🌐 Todas las cuentas</span>
          <span style={{color:'#06d6a0',fontWeight:800}}>$22,215.50</span>
        </div>
      </div>
    )
  },
  {
    icon: '📤', title: 'Importa y Exporta tus Datos',
    desc: 'Sube tu historial en CSV o descarga tus operaciones en CSV y PDF cuando quieras.',
    preview: (
      <div style={{fontSize:'11px'}}>
        <div style={{marginBottom:'6px',padding:'8px',background:'#0b1120',borderRadius:'6px',display:'flex',alignItems:'center',gap:'8px'}}>
          <span style={{fontSize:'16px'}}>📥</span>
          <div>
            <div style={{color:'#e2e8f0',fontWeight:600}}>Importar CSV</div>
            <div style={{color:'#64748b',fontSize:'9px'}}>Sube tu historial de operaciones</div>
          </div>
        </div>
        <div style={{display:'flex',gap:'6px'}}>
          <div style={{flex:1,padding:'8px',background:'#0b1120',borderRadius:'6px',textAlign:'center'}}>
            <div style={{fontSize:'16px',marginBottom:'2px'}}>📊</div>
            <div style={{color:'#94a3b8',fontSize:'9px'}}>Exportar CSV</div>
          </div>
          <div style={{flex:1,padding:'8px',background:'#06d6a018',border:'1px solid #06d6a030',borderRadius:'6px',textAlign:'center'}}>
            <div style={{fontSize:'16px',marginBottom:'2px'}}>🧾</div>
            <div style={{color:'#06d6a0',fontSize:'9px',fontWeight:600}}>Reporte PDF</div>
          </div>
        </div>
      </div>
    )
...
const PLANS = [
  ...
  { name: 'Pro Mensual', price: '$139', period: 'MXN/mes', features: ['Operaciones ilimitadas', 'Analytics completos', 'Calendar heatmap', 'Backtesting avanzado', 'Múltiples cuentas + vista combinada', 'Importar / Exportar CSV', 'Reportes en PDF', 'P&L mensual detallado'], cta: 'Empezar Pro', highlight: false },
...
export default function Landing({ onLogin, onSignup, onShowTerms, onShowPrivacy, onShowContact }) {
  return (
    <div style={{ minHeight:'100vh', background:'#0b1120', color:'#e2e8f0', fontFamily:"'Inter', system-ui, sans-serif" }}>
      ...
      <section style={{ padding:'40px 20px' }}>
        <div style={{ maxWidth:'720px', margin:'0 auto', padding:'28px 32px', borderRadius:'16px', background:'linear-gradient(135deg, #06d6a015, #0b112000)', border:'1px solid #06d6a030', display:'flex', alignItems:'center', gap:'20px', flexWrap:'wrap', justifyContent:'space-between' }}>
          <div>
            <div style={{ fontSize:'20px', fontWeight:800, marginBottom:'6px' }}>🎁 Invita a un amigo, ganen los dos</div>
            <p style={{ color:'#94a3b8', fontSize:'13px', maxWidth:'420px' }}>Comparte tu link de referido. Cuando tu invitado se haga Pro, ambos reciben 1 mes de Operata Pro gratis.</p>
          </div>
          <button onClick={onSignup} style={{ padding:'11px 24px', borderRadius:'9px', border:'1px solid #06d6a0', background:'transparent', color:'#06d6a0', fontWeight:700, fontSize:'13px', cursor:'pointer', flexShrink:0 }}>
            Crear cuenta y obtener mi link →
          </button>
        </div>
      </section>
      ...
      <footer style={{ borderTop:'1px solid #1e293b', padding:'32px 40px 24px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:'24px', marginBottom:'20px' }}>
          <div>
            <span style={{ fontWeight:700, color:'#06d6a0', fontSize:'16px' }}>Operata</span>
            <p style={{ color:'#475569', fontSize:'12px', marginTop:'6px', maxWidth:'260px' }}>Diario de trading inteligente para operar con datos, no con intuición.</p>
          </div>

          <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
            <span style={{ color:'#64748b', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'2px' }}>Síguenos</span>
            <a href="https://instagram.com/operata.tech" target="_blank" rel="noopener noreferrer" style={{ color:'#94a3b8', fontSize:'13px', textDecoration:'none' }}>Instagram</a>
            {/* Agregar aquí el link de Facebook cuando lo tengamos:
            <a href="https://facebook.com/TU_USUARIO" target="_blank" rel="noopener noreferrer" style={{ color:'#94a3b8', fontSize:'13px', textDecoration:'none' }}>Facebook</a> */}
          </div>

          <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
            <span style={{ color:'#64748b', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'2px' }}>Legal</span>
            <button onClick={onShowTerms} style={{ color:'#94a3b8', fontSize:'13px', background:'none', border:'none', padding:0, textAlign:'left', cursor:'pointer' }}>Términos y condiciones</button>
            <button onClick={onShowPrivacy} style={{ color:'#94a3b8', fontSize:'13px', background:'none', border:'none', padding:0, textAlign:'left', cursor:'pointer' }}>Aviso de privacidad</button>
          </div>

          <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
            <span style={{ color:'#64748b', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'2px' }}>Ayuda</span>
            <button onClick={onShowContact} style={{ color:'#94a3b8', fontSize:'13px', background:'none', border:'none', padding:0, textAlign:'left', cursor:'pointer' }}>Contacto y soporte</button>
          </div>
        </div>

        <div style={{ borderTop:'1px solid #1e293b', paddingTop:'16px', fontSize:'12px', color:'#475569' }}>
          © 2026 Operata · Tu diario de trading inteligente
        </div>
      </footer>
    </div>
  )
