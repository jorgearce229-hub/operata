...
export default function Dashboard({ session, profile, onProfileUpdate, theme: themeProp, onThemeChange: onThemeChangeProp, onShowSettings, onShowUpgrade, onShowTerms, onShowPrivacy, onShowContact }) {
  ...
  useEffect(() => {
    const restoreActiveAccount = async () => {
      ...
      if (data?.length) {
        ...
        if (lastId === 'all') {
          const totalCapital = data.reduce((s, a) => s + (a.balance || 0), 0)
          const currencies = [...new Set(data.map(a => a.currency))]
          setActiveAccount({ id: 'all', name: 'Todas las cuentas', currency: currencies.length === 1 ? currencies[0] : '', balance: totalCapital })
        } else {
  ...
  const loadTrades = async () => {
    ...
    // La vista "Todas las cuentas" (id === 'all') no filtra, muestra todo combinado.
    if (activeAccount?.id && activeAccount.id !== 'all') query = query.eq('account_id', activeAccount.id)
