    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: bg.bg, color: bg.text }}>
      ...
      <div style={{ width: sidebarOpen ? '220px' : '64px', background: bg.card, borderRight: `1px solid ${bg.border}`, display: 'flex', flexDirection: 'column', transition: 'width 0.2s', flexShrink: 0 }}>
        ...
        {sidebarOpen && <div style={{ padding: '16px', borderTop: `1px solid ${bg.border}`, fontSize: '11px' }}>
          ...
          <div style={{ color: bg.dim, fontSize: '11px', display: 'flex', justifyContent: 'center' }}>
            <span style={{ fontSize: '11px', color: bg.dim }}>👤 {profile?.full_name || 'Mi cuenta'}</span>
          </div>
        </div>}
      ...
      <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', background: bg.bg }}>
            ...
            {showUserMenu && (
              <div style={{
                ...
                <div style={{ padding: '10px 12px', marginBottom: '4px', borderBottom: `1px solid ${bg.border}` }}>
                  <div style={{ fontSize: '13px', fontWeight: 600, color: bg.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{profile?.full_name || 'Usuario'}</div>
                ...
                {/* Menu items */}
                {[
                  { icon: '⚙️', label: 'Configuración', action: () => { setShowSettings(true); setShowUserMenu(false) } },
                  { icon: '⏏', label: 'Cerrar sesión', action: handleSignOut, danger: true },
                ].map((item, i) => (
                  <div key={i} onClick={item.action} style={{
                    display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px',
                    borderRadius: '8px', cursor: 'pointer', fontSize: '13px',
                    color: item.danger ? bg.red : bg.text,
                    transition: 'background 0.1s'
                  }}
                  onMouseEnter={e => e.currentTarget.style.background = item.danger ? bg.redBg : bg.bg}
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '24px' }}>
          ...
          <div className="fade-in" key={showSettings ? 'settings' : tab}>
            {showSettings
              ? <Settings profile={profile} userId={session.user.id} onProfileUpdate={onProfileUpdate} theme={theme} onThemeChange={applyTheme} isPro={isPro} />
              : <>
