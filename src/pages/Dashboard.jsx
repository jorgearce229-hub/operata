...
export default function Dashboard({ session, profile, onProfileUpdate, theme: themeProp, onThemeChange: onThemeChangeProp, onShowSettings, onShowUpgrade, onShowTerms, onShowPrivacy }) {
            ...
            <div style={{ padding: '12px 16px 8px' }}>
              ...
              {activeAccount && (
                <div style={{ textAlign: 'center', padding: '6px 0 4px', borderTop: `1px solid ${bg.border}`, marginTop: '4px' }}>
                  ...
                  <div style={{ fontSize: '10px', color: bg.dim }}>{activeAccount.currency} {(activeAccount.balance||0).toLocaleString('es-MX',{minimumFractionDigits:2})}</div>
                </div>
              )}
            </div>
          )}
      ...
      <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', background: bg.bg }}>
          ...
          <div style={{ position: 'relative' }} data-usermenu="true">
            ...
            {showUserMenu && (
              <div data-usermenu="true" style={{
                ...
                <div data-usermenu="true" onClick={() => { window.open('mailto:operata.soporte@gmail.com?subject=Reporte%20desde%20Operata', '_blank'); setShowUserMenu(false) }} style={{
                  display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px',
                  borderRadius: '8px', cursor: 'pointer', fontSize: '13px', color: bg.muted
                }}
                onMouseEnter={e => e.currentTarget.style.background = bg.bg}
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '24px' }}>
          ...
          <div className="fade-in" key={tab}>
            ...
            {tab === 'analytics' && <Analytics trades={trades} isPro={isPro} profile={profile} onUpgrade={() => onShowUpgrade && onShowUpgrade()} />}
