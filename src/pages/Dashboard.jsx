...
export default function Dashboard({ session, profile, onProfileUpdate, theme: themeProp, onThemeChange: onThemeChangeProp, onShowSettings, onShowUpgrade, onShowTerms, onShowPrivacy, onShowContact }) {
  ...
  const loadTrades = async () => {
    ...
    // Si hay una cuenta activa, mostramos sus operaciones + las operaciones antiguas
    // que todavía no tienen cuenta asignada (para no ocultar historial existente).
    if (activeAccount?.id) query = query.or(`account_id.eq.${activeAccount.id},account_id.is.null`)
