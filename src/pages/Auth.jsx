    ...
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          <svg viewBox="0 0 128 128" width="48" height="48" style={{ margin: '0 auto 12px' }}>
    ...
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '16px', padding: '32px' }}>
          ...
          {mode === 'signup' && (
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', margin: '12px 0 16px', padding: '12px', background: C.bg, borderRadius: '8px', border: `1px solid ${termsAccepted ? C.accent + '40' : C.border}` }}>
              <input type="checkbox" id="terms" checked={termsAccepted} onChange={e => setTermsAccepted(e.target.checked)}
                style={{ marginTop: '2px', cursor: 'pointer', accentColor: C.accent, width: '16px', height: '16px', flexShrink: 0 }} />
              <label htmlFor="terms" style={{ fontSize: '12px', color: C.muted, cursor: 'pointer', lineHeight: 1.5 }}>
                He leído y acepto los{' '}
                <span style={{ color: C.accent, textDecoration: 'underline', cursor: 'pointer' }}
                  onClick={e => { e.preventDefault(); window.open('/?terms=true', '_blank') }}>Términos y Condiciones</span>
                {' '}y la{' '}
                <span style={{ color: C.accent, textDecoration: 'underline', cursor: 'pointer' }}
                  onClick={e => { e.preventDefault(); window.open('/?privacy=true', '_blank') }}>Política de Privacidad</span>
              </label>
            </div>
          )}
          {error && <div style={{ padding: '10px', background: C.redBg, border: `1px solid ${C.red}30`, borderRadius: '8px', color: C.red, fontSize: '13px', marginBottom: '16px' }}>{error}</div>}
